...
/* Again modified by Jones for Jones/VisitorLog */
...
class Useragent {
    ...
    public function __construct($agent)
    {
		if(!self::$config)
		{
			self::$config = Config::get('visitor-log::useragent');
		}
		...
		if (is_array(self::$browsers) AND count(self::$browsers) > 0)
		{
			foreach (self::$browsers as $key => $val)
			{
				if (preg_match("|".preg_quote($key).".*?([0-9\.]+)|i", self::$agent, $match))
				{
					...
					// Fix for eg Opera
					if (preg_match("|Version.*?([0-9\.]+)|i", self::$agent, $match))
						self::$version = $match[1];
					self::$browser = $val;
					self::_set_mobile();
					return TRUE;
